...
import { Search, Sparkles, Star, BookOpen, Clock } from "lucide-react";
...
export default function Explore() {
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="relative w-full md:w-80">
          ...
          <Input 
            ...
            className="pl-10 h-12 rounded-xl bg-white border-slate-200 focus-visible:ring-[#0B56D5]"
            value={searchQuery}
      ...
      {/* Categories Filter (Dùng Badge của shadcn) */}
      <div className="flex items-center gap-2 overflow-x-auto pb-2 custom-scrollbar">
        {CATEGORIES.map(category => (
          <Badge 
            key={category}
            variant={activeTag === category ? "default" : "secondary"}
            className={`cursor-pointer px-4 py-2 text-sm rounded-lg whitespace-nowrap transition-all ${
              activeTag === category ? "bg-[#0B56D5] hover:bg-blue-700" : "bg-slate-100 hover:bg-slate-200 text-slate-600 font-medium"
            }`}
      ...
      {/* Danh sách Card (Dùng Card của shadcn) */}
            ...
            <Card className="rounded-3xl overflow-hidden cursor-pointer group hover:shadow-[0_20px_40px_rgba(11,86,213,0.1)] transition-all border-slate-100 h-full flex flex-col">
              <CardContent className="p-3 flex-1 flex flex-col">
                <div className={`w-full h-40 ${course.bg} rounded-2xl relative flex items-center justify-center mb-4`}>
                  <div className="absolute top-3 left-3 bg-white/20 backdrop-blur-md text-white text-[10px] font-bold px-2 py-1 rounded-lg">
                    {course.category}
                  </div>
